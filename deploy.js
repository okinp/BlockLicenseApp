...
const settings = fs.readFileSync('truffle.js');
const network = JSON.parse(settings.network.development);
...
const input = fs.readFileSync('./contracts/Licenses.sol');
const output = solc.compile(input.toString(), 1);
const bytecode = output.contracts['Licenses'].bytecode;
const abi = JSON.parse(output.contracts['Licenses'].interface);
fs.writeFileSync('./Licenses.abi', abi, 'utf8');
